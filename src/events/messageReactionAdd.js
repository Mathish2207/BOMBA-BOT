...
module.exports = {
  ...
  async execute(reaction, user) {
    ...
    if (rmError) {
      console.error('Erreur Supabase (messageReactionAdd) :', rmError);
      return;
    }
    ...
    if (mappingError) {
      console.error('Erreur Supabase (messageReactionAdd) :', mappingError);
      return;
    }
    if (!mapping) return;
    ...
    try {
      ...
      console.error(`Impossible d'attribuer le rôle ${mapping.role_id} à ${user.id} :`, error);
    }
  },
};
